...
void	renderer_draw_controls_scaling(const t_scene_interactor* interactor, const t_mat4f* vp)
{
	...
	mvp = get_mvp(interactor->actor_selected, &b[0], vp, &rt[0], 1.15f + r.x);
	renderer_draw_cube(&mvp, &colors[0], 0.15f);
	mvp = get_mvp(interactor->actor_selected, &b[1], vp, &rt[1], 1.15f + r.y);
	renderer_draw_cube(&mvp, &colors[1], 0.15f);
	mvp = get_mvp(interactor->actor_selected, &b[2], vp, &rt[2], 1.15f + r.z);
	renderer_draw_cube(&mvp, &colors[2], 0.15f);
}
