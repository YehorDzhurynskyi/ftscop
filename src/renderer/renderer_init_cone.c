...
t_bool	renderer_init_cone_program(t_gfx_program_cone *p)
{
	...
	p->u_location_height = glGetUniformLocation(p->id, "u_height");
	...
	if (p->u_location_mvp < 0 || p->u_location_nsegments < 0 ||
		p->u_location_height < 0 || p->u_location_color_tint < 0 ||
		p->u_location_radius < 0)
	{
